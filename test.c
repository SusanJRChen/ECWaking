...
/*
 ...
 content=$(curl freegeoip.net/json/[IPADDRESS])
 ...
 content=$(curl query.yahooapis.com/v1/public/yql -d q="select woeid from geo.places where text=\"([LONGITUDE], [LATITUDE])\"" -d format=json)
 echo "$content" > "$content.txt"
 ...
 content=$(curl query.yahooapis.com/v1/public/yql -d q="select wind from weather.forecast where woeid= [WOEID]" -d format=json)
 echo "$content" > "$content.txt"
